...
// creo la instancia  de la mother
    ...
    if (Math.abs(diferencia) > 10) {
      ...
      const transicion = () => {
        try {
          ...
          mbController.setAllColorNow(`hsl(${color_salto},100%,50%)`);
        } catch {
          // si no puedo por algun motivo, entonces vuelvo a iniciar la funcion
          // principal
          console.log(color_salto, "gradient");
          setTimeout(() => {
            main();
          }, 50);
          return null;
        ...
        if (contador <= steps) {
          ...
          try {
            mbController.setAllColorNow(`hsl(${contents},100%,50%)`);
          } catch {
            console.log("saliendo", contents);
            setTimeout(() => {
              main();
            }, 50);
            return null;
      ...
      try {
        ...
        console.log("poca diff", contents);
